...
local function main()
    ...
    SaveInstance.Init = function()
        ...
        local UploadTelegram = AddCheckbox("Upload to Telegram", SaveInstanceArgs.UploadTelegram)
        UploadTelegram.OnInput:Connect(function()
            SaveInstanceArgs.UploadTelegram = UploadTelegram.Toggled
        end)
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            if s then
                ...
                if SaveInstanceArgs.UploadTelegram then
                    task.spawn(function()
                        window:SetTitle("Save Instance - Uploading to Telegram")
                        local http = service.HttpService
                        local filePath = fileName..".rbxlx"
                        local fileData = nil
                        local successFile, errFile = pcall(function()
                            fileData = env.readfile(filePath)
                        end)
                        ...
                        if successFile and fileData and #fileData > 0 then
                            ...
                            local successSend, sendResult = pcall(function()
                                ...
                                if not parsed.ok then
                                    error("Telegram Error: " .. (parsed.description or "Unknown"))
                                end
                            end)
                            ...
                            if successSend then
                                ...
                                warn("Telegram upload failed, falling back to text message: " .. tostring(sendResult))
                            end
                        else
                            warn("Failed to read file for Telegram upload: " .. tostring(errFile))
                        end
                            ...
                            pcall(function()
                                http:RequestAsync({
                                    ...
                                    Body = http:JSONEncode({
                                        ...
                                        text = "SaveInstance: Successfully saved '"..fileName..".rbxlx' but failed to upload to Telegram. File might be too large or read error occurred."
                                    })
                                })
                            end)
                        end
